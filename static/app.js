class FitnessApp {
    ...
    updateProfileScreen() {
        ...
        // Ajouter un délai pour s'assurer que le DOM est rendu
        setTimeout(() => {
    ...
    setupNameEditor() {
        ...
        if (!editBtn) {
            ...
            button.innerHTML = '✏️';
            button.title = 'Modifier le nom';
            nameElement.parentNode.appendChild(button);
            
            button.addEventListener('click', () => this.startNameEditing());
        } else {
            editBtn.onclick = () => this.startNameEditing();
        }
    }

    startNameEditing() {
        const nameElement = document.getElementById('player-name');
        const currentName = this.playerData.playerName;
        
        nameElement.innerHTML = `
            <input type="text" id="name-edit-input" value="${currentName}" maxlength="20" style="width: 200px; text-align: center;">
            <button onclick="window.fitnessApp.saveName()" style="background: var(--secondary-color); margin-left: 5px;">✓</button>
            <button onclick="window.fitnessApp.cancelNameEdit()" style="background: var(--accent-color); margin-left: 5px;">✗</button>
        `;
    ...
    saveName() {
        ...
        this.savePlayerData();
        this.updateProfileScreen();
        this.showNotification('Nom modifié avec succès !', 'success');
    }
    ...
    setupResetButton() {
        ...
        if (resetButton) {
            ...
            console.error('Bouton reset NON TROUVÉ dans le DOM !');
            console.log('Éléments disponibles:', document.querySelectorAll('#profile-screen *'));
        }
    }

    // Nouvelle méthode intégrée dans la classe
    async resetProgress() {
        console.log('resetProgress appelée !');
        
        try {
            // Demande de confirmation à l'utilisateur
            const confirmed = confirm(
                '⚠️ ATTENTION ⚠️\n\n' +
                'Tu es sur le point de supprimer TOUTE ta progression :\n' +
                '• Ton niveau et tes XP\n' +
                '• Tes statistiques d\'entraînement\n' +
                '• Ton historique complet\n' +
                '• Tes succès débloqués\n\n' +
                'Cette action est IRRÉVERSIBLE !\n\n' +
                'Es-tu vraiment sûr(e) de vouloir tout recommencer à zéro ?'
            );
            ...
            const doubleConfirmed = confirm(
                'Dernière chance !\n\n' +
                'Confirmes-tu définitivement la suppression de TOUTE ta progression ?\n\n' +
                'Clique sur OK pour confirmer.'
            );
            
            if (!doubleConfirmed) {
                console.log('Reset annulé à la double confirmation');
                return;
            }
            
            console.log('Début du processus de reset...');
            
            // Afficher une notification pendant le processus
            this.showNotification('🔄 Réinitialisation en cours...', 'success');
            
            // Appel à l'API de reset
            console.log('Appel API /api/reset-progress');
            ...
            if (result.status === 'success') {
                console.log('Reset réussi côté serveur');
